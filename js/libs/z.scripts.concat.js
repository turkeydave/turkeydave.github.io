		...
		function tweenTo(i) {
			...
			TweenLite.to(timeline, Math.abs(positions[i] - timeline.time()), {time:positions[i], ease:Linear.easeNone, onComplete:function() {
                    ...
                    if(positionIndex === 10){
                        ...
                        // handle coming back (just do no op)
                        var test = $('#myChart');
...
(function($) {
  ...
  tl.add(slideAboutMe);
    ...
    slideEpic.add(TweenMax.to($('body'),0.5,{backgroundColor:'#FF4316'}));
    slideEpic.add(TweenMax.from($('#slide-epic .slide-epic-content').css('position','relative'),0.75,{opacity:0,top:150,rotationY:450, delay:-0.25,transformOrigin:"50% 50%", perspective:2000,ease:Expo.easeOut}));
    tl.add(slideEpic);

    // // SLIDE ======================================================================== //
    var slideEpic2 = new TimelineMax();
    // outro prev slide
    slideEpic2.add(TweenMax.to($('#slide-epic'),0.25,{opacity:0}),0);
    slideEpic2.add(TweenMax.to($('#slide-epic'),0,{immediateRender:false,css:{display:'none'}}));
    // intro
    slideEpic2.add(TweenMax.to($('#slide-epic2'),0,{immediateRender:false,css:{display:'block'}}));
    slideEpic2.add(TweenMax.to($('body'),0.5,{backgroundColor:'#FF556B'}));
    slideEpic2.add(TweenMax.from($('#slide-epic2 img').css('position','relative'),0.75,{opacity:0,top:150,rotationY:450, delay:-0.25,transformOrigin:"50% 50%", perspective:2000,ease:Expo.easeOut}));
    tl.add(slideEpic2);
